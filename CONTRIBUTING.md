...
- **One change = one branch = one PR.** A PR normally carries one task,
  ...
  *authors* a rule rather than
  ...
  [Pipeline](docs/product/pipeline/authoring.md#two-ways-a-permanent-doc-changes) for
...
per WritRun's own [human gates](docs/product/pipeline/gates.md), and
...
- **Front matter stays canonical** — one field per line, `key: value`,
  ...
  [Front matter is canonical](docs/technical/README.md#front-matter-is-canonical).
