...
# `status:pending` used to claim "Task exists" on issues where the task
...
forge_told "pending no longer claims a task that does not exist" \
  "-f name=status:pending -f color=fbca04 -f description=In the queue, with a spec it references not yet approved"
