...
# `credit_ai: false` says everything the agent writes into git and the
...
check "a clean range under credit_ai false passes" 0 "credit_ai is honoured" \
...
check "a co-author trailer is flagged, and the line is named" 1 \
  "carries platform credit while credit_ai is false" \
  -- bash "$CHECK_OBSERVANCE" main...HEAD
...
allowing
commit_all
check "under credit_ai true the check does not run" 0 \
