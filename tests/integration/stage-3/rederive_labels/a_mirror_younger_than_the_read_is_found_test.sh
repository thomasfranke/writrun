...
# miss re-reads before it concludes anything
...
check "an id the first read missed is labelled from the second" 0 \
  "task-0009 → status:ready" \
  -- bash "$REDERIVE_LABELS" o/r task-0008 task-0009
...
# read the same way — so it goes stale the same way.
setup_forge
