...
mkdir -p a-dir/nested kit/a-dir/nested
printf 'the root\n'    > a-dir/nested/settings.json
printf 'the root\n'    > a-dir/other.txt
printf 'the kit own\n' > kit/a-dir/nested/settings.json
printf 'a-dir\n'                     > mirrors.txt
...
if [ "$(cat kit/a-dir/nested/settings.json)" = "the kit own" ]; then
  echo "ok    the exception survives a directory mirror"; pass=$((pass + 1))
else
  echo "FAIL  the exception survives a directory mirror"
  echo "      | the kit holds: $(cat kit/a-dir/nested/settings.json)"
  fail=$((fail + 1))
fi

if [ "$(cat kit/a-dir/other.txt)" = "the root" ]; then
...
rm -f kit/a-dir/nested/settings.json
