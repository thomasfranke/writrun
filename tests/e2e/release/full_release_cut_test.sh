...
# and fails everything else, so nested cases that rely on an unreachable
# forge (list_tasks' warning) still see one.
printf '#!/usr/bin/env bash\ncase "${1:-}" in release|auth) ;; *) exit 1 ;; esac\necho "gh $*" >> "%s/gh.log"\n' "$WORK" > "$WORK/stub-bin/gh"
chmod +x "$WORK/stub-bin/gh"
export PATH="$WORK/stub-bin:$PATH"
export WRITRUN_E2E_RUNNING=1
unset MAKE
...
if [ "$code" -eq 0 ] &&
   ...
   grep -q 'gh release create v0.0.11' "$WORK/gh.log"; then
  echo "ok    a real make release cuts, syncs, tests, commits, tags, pushes, publishes"; pass=$((pass + 1))
else
  echo "FAIL  a real make release cuts, syncs, tests, commits, tags, pushes, publishes"
