...
check "a chosen slug is taken verbatim" 0 "task-0001-stamp-queue-dates.md" \
  -- bash "$NEW_SH" task "Stamp queued and merged on the task" --origin rule --slug stamp-queue-dates
...
check "no --slug still derives from the title" 0 "task-0002-second-thing-entirely.md" \
  -- bash "$NEW_SH" task "Second thing entirely, and then some" --origin rule
...
check "a slug another id already carries is allowed" 0 "task-0003-stamp-queue-dates.md" \
  -- bash "$NEW_SH" task "A different task about the same area" --origin rule --slug stamp-queue-dates
...
check "a long slug is shape, not taste" 0 \
  ...
  -- bash "$NEW_SH" task "Long" --origin rule --slug one-two-three-four-five-six
