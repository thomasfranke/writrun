...
# the same five fields step 0 packs — one short and the title slides into
# the pause slot, printed as a reason nobody derived. The spec token is
# the fifth, and it is the one a second packer is likeliest to forget.
setup
...
check "the taken task is named beside its own title" 1 \
  "@dana  *spec-0009:approved  *Test task task-0007" -- bash "$LIST_TASKS"
