...
task_id=$(printf '%s' "$branch" | sed -n 's|^[a-z]*/\{0,1\}\(task-[0-9][0-9]*\).*|\1|p')
if [ -z "$task_id" ]; then
  ...
  if [ -n "$sref" ]; then
    sf="work/specs/$(printf '%s' "$sref" | tr '[:upper:]' '[:lower:]').md"
    [ -f "$sf" ] && task_id=$(sed -n 's/^task_ref: *//p' "$sf" | head -n1 \
      | sed 's/[[:space:]]*$//')
  fi
  if [ -z "$task_id" ]; then
    n=$(printf '%s' "$branch" | sed -n 's|^task/0*\([0-9][0-9]*\).*|\1|p')
    [ -n "$n" ] && task_id=$(printf 'task-%03d' "$n")
  fi
fi
...
while IFS="$TAB" read -r num istate labels tb bb; do
  ...
  case "$t" in
    "$task_id "*) issue_num="$num"; issue_labels="$labels"; break ;;
  esac
done <<EOF
$ISSUES
EOF
...
while IFS="$TAB" read -r fstatus fname fpatch; do
  ...
  printf '%s' "$fname" | tr '[:upper:]' '[:lower:]' \
    | grep -qxF "work/tasks/${task_id}.md" || continue
