...
# states that none was (docs/product/pipeline.md#declaring-derived-work).
#
...
perm=$(git diff --name-only "$RANGE" -- docs \
...
added=$(git diff --name-only --diff-filter=A "$RANGE" -- 'work/tasks/task-*.md' || true)
if [ -n "$added" ]; then
...
echo "see docs/product/pipeline.md#declaring-derived-work." >&2
exit 1
