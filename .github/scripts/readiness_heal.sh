...
# regeneration changed the kit, commit it and push; when it changed
...
#   Run from the repository root after `make kit-sync`. Pushes
...
# kit/ is invisible to `git diff`, and a heal blind to it would
# ship the kit without the file while every check stayed green.
if [ -z "$(git status --porcelain -- kit)" ]; then
...
git add -A kit
git commit -m "chore(setup): sync the kit with the root it mirrors" \
...
echo "healed — kit sync committed to ${BRANCH}"
